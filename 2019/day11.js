...
async function pipe(out) {
    ...
    if (isColor) {
        // console.log('Setting ' + pos.join() + ' to ' + out);
        field[pos.join()] = out;
    } else {
        ...
        // console.log('Turning ' + (out ? 'right' : 'left') + ', dir is ' + directions[dirIndex].join());
        ...
        // Return fields color
        return_value = field[pos.join()] || 0; // default black
    }
    isColor = !isColor;
...
comp.run(prgm, false, undefined, pipe).then(_ => {
    print(field);
    // console.log(field);
...
function print(map) {
    ...
    vals.forEach((coord, i) => {
        ...
        console.log(`<rect x="${10*coord[0]}" y="${10*coord[1]}" width="10" height="10" fill="${colors[i] ? 'white' : 'black'}" />`);
    })
    console.log('</svg>');
}
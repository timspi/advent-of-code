...
async function pipe(data) {
    map += data < 256 ? String.fromCharCode(data) : data;
}
...
//             if ('<>^v'.includes(data[y][x])) pos = [x, y];
//             if (
//                 data[y][x] === '#' &&
//                 data[y+1][x] === '#' &&
//                 data[y-1][x] === '#' &&
...
//     let dir = robotDir(pos);
//     console.log('pos,dir', pos, dir);
//     let cmds = [], walkCounter = 0;
...
//         } else {
//             // Check left
//             let dirLeft = turn(dir, 'left');
//             if (get(add(pos, dirLeft)) == '#') {
//                 dir = dirLeft;
//                 if (walkCounter) {
//                     cmds.push(walkCounter);
//                     walkCounter = 0;
//                 }
//                 cmds.push('L');
//             } else {
//                 // Check right
//                 let dirRight = turn(dir, 'right');
//                 if (get(add(pos, dirRight)) == '#') {
//                     dir = dirRight;
//                     if (walkCounter) {
//                         cmds.push(walkCounter);
//                         walkCounter = 0;
...
let input_values = convertToASCII(cmd_lines.join('\n') + '\n');
comp.run(prgm, false, input_values, pipe).then(() => {
    console.log(map);
});
...
function turn(dir, val) {
    let offset = val === 'right' ? 1 : 3;
    let i = directions.findIndex(el => el.vec[0] === dir[0] && el.vec[1] === dir[1])
    return directions[ (i + offset) % 4 ].vec;
}

function get(vec) {
    let line = data[ vec[1] ];
    return line && line[ vec[0] ];
}
